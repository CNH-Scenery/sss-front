...
class App extends React.Component {
  ...
  runBacktest=async()=>{ if(this.state.backtesting||!this.state.strategy)return; const codeId=this.state.strategy.codeId; if(!codeId){ this.setState({backtestError:'백엔드 code_id가 없습니다. 먼저 코드를 생성(코드화)해 주세요.'}); return; } const market=normalizeUpbitMarket(this._backtestMarketInput.current?.value||this.state.backtestMarketDraft); const range={startDate:this._backtestStartInput.current?.value||this.state.backtestStartDate,endDate:this._backtestEndInput.current?.value||this.state.backtestEndDate}; this.setState({backtesting:true,backtestError:'',backtestMarket:market,backtestMarketDraft:market,backtestStartDate:range.startDate,backtestEndDate:range.endDate}); try{ const start=range.startDate?range.startDate+'T00:00:00':null; const end=range.endDate?range.endDate+'T23:59:59':null; const r=await runBacktestApi({codeId,market,timeframe:'1d',start,end,lookback:400}); if(this._unmounted)return; const rangeKey=market+':'+(r.period_start||start||'')+':'+(r.period_end||end||''); const bt={rangeKey,candles:r.candles||[],markers:r.markers||[],eq:r.eq||[],bh:r.bh||[],metrics:r.metrics,summary:{market,rangeKey,rangeCandleCount:(r.candles||[]).length,dataFrom:r.period_start,dataTo:r.period_end,from:r.period_start,to:r.period_end,requestedFrom:range.startDate,requestedTo:range.endDate,source:'백엔드 백테스트('+(r.timeframe||'1d')+') · 생성 코드 실행'}}; this.setState({backtesting:false,backtest:bt,backtestError:''}); }catch(error){ if(this._unmounted)return; this.setState({backtesting:false,backtestError:error?.message||'백테스트 실패'}); } };
